...
private:
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_Changed;
...
private:
    ...
    inline void initializePointCloud()
    {
		...
		cloud_Changed = boost::make_shared<pcl::PointCloud<pcl::PointXYZRGB>>();
		cloud_Changed->width = static_cast<uint32_t>(depthWidth);
		cloud_Changed->height = static_cast<uint32_t>(depthHeight);
		cloud_Changed->points.resize(cloud->height * cloud->width);
		cloud_Changed->is_dense = false;
    ...
    inline void updatePointCloud()
    {
		...
		//pcl::io::savePCDFile("KinectV2_cpature.pcd", *cloud, true); // Binary format
		//exit(1);
    }
